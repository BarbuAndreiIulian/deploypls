...
const Post = ({ id, username, userImg, img, caption }) => {
  ...
  return (
    <div className="bg-white my-7 border rounded-sm">
      ...
      {/* Btns */}
      {session && (
        <div className="flex justify-between px-4 py-4 pb-0">
          <div className="flex space-x-4">
            {hasLiked ? (
              <HiHeart className="btn text-red-500" onClick={likePost} />
            ) : (
              <HiOutlineHeart className="btn" onClick={likePost} />
            )}
      ...
      {/* Comments */}
      {comments.length > 0 && (
        <div className=" h-20 overflow-y-scroll scrollbar-thumb-black scrollbar-thin   ml-8 mr-8 ">
      ...
      {session && (
        <form className="flex  items-center  p-4">
          <HiOutlineEmojiHappy className="w-7 h-7" />
          <input
            type="text"
            value={comment}
